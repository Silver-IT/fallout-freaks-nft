    ...
    Whitelist
{
    ...
    struct MintDetails {
        uint32 originalId; // The JF Genesis Token Id used to mint
        uint32 level; // The level of burned item used
        uint32 mintType; // If the token was directly minted (0: Random Mint), or indirectly (1: Crystal Mint)
        uint32 mutationId; // The mutationId
        uint128 aux; // This is so we have access to the rest of the storage slot if required (this is more for standardisation going forwards)
    }
    ...
    address public BENEFICIARY1 = 0xFA60C3295Ea671E365B9A242551Cf1Ff314Bf045;
    address public BENEFICIARY2 = 0x8e5F332a0662C8c06BDD1Eed105Ba1C4800d4c2f;
    address public BENEFICIARY3 = 0x954BfE5137c8D2816cE018EFd406757f9a060e5f;
    address public BENEFICIARY4 = 0xd196e0aFacA3679C27FC05ba8C9D3ABBCD353b5D;

    uint256[] public BENEFICIARIES_PRIMARY = [3100, 3100, 1800, 2000];
    ...
    // Jungle Freaks Crystal contract
    IFalloutCrystal private _jfcContract; // tokenid => level
    ...
    modifier checkOriginalAndCrystalUsage(
        ...
        if (_jfgContract.ownerOf(originalTokenId_) != _msgSender()) {
    ...
    modifier enoughEthProvided(uint8 quantity_) {
        uint256 discount;
        if (IERC721(_jfgContract).balanceOf(_msgSender()) > 0) {
            discount += 0.02 ether;
        }
    ...
    // Reserve Random Mint
    function setReservedMintSupply(uint256 reservedMintSupply_)
        public
        onlyAdmin
    {
        _setReservedMintSupply(reservedMintSupply_);
    }
    ...
    function setPrimarySalesSplits() public onlyAdmin {
        ...
        addresses[3] = BENEFICIARY4;
        setBeneficiaries(addresses, BENEFICIARIES_PRIMARY);
    }
    ...
    function startCrystalMutationMint() external onlyAdmin {
