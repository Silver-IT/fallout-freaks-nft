...
export default function suite() {
  ...
  it("should burn a token because it's called by a pre authorized user", async () => {
    await expect(mockContract.connect(ctx.user7).burn(ctx.user1.address, 1, 2)).to.be.reverted;

    await expect(mockContract.connect(ctx.user8).burn(ctx.user1.address, 1, 2)).to.emit(
      mockContract,
      "TransferSingle"
    );
    ...
    await expect(mockContract.connect(ctx.user9).burn(ctx.user1.address, 1, 1)).to.emit(
      mockContract,
      "TransferSingle"
    );
