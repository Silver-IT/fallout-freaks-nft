...
export default function suite() {
  ...
  describe("When receiving ether", async () => {
    it("should allow contract to receive ether", async () => {
      ...
      await expect(
        ctx.owner.sendTransaction({ to: ctx.falloutContract.address, value })
      ).to.not.be.reverted;
    });
  });
  ...
  describe("When setting withdrawal addresses", async () => {
    it("should return the correct default withdrawal address when no withdrawal address has been set for Jungle Freaks", async () => {
      expect((await ctx.falloutContract.beneficiaries(0)).wallet).to.eql(
    ...
    it("should return the correct default withdrawal address when no withdrawal address has been set for Scott A", async () => {
      expect((await ctx.falloutContract.beneficiaries(1)).wallet).to.eql(
    ...
    it("should return the correct default withdrawal address when no withdrawal address has been set for New Wallet", async () => {
      expect((await ctx.falloutContract.beneficiaries(2)).wallet).to.eql(
    ...
    it("should return the correct default withdrawal address when no withdrawal address has been set for Massless", async () => {
      expect((await ctx.falloutContract.beneficiaries(3)).wallet).to.eql(
  ...
  describe("When withdrawing Ether Funds", async () => {
    it("should fail to withdraw when balance is zero", async () => {
      await expect(ctx.falloutContract.withdrawEth()).to.be.revertedWith(
        "ZeroBalance"
      );
    });

    it("should allow anyone to withdrawEth()", async () => {
      const value = ethers.utils.parseEther("1");
      ...
      await expect(ctx.falloutContract.withdrawEth()).to.not.be.reverted;

      await expect(
        ctx.owner.sendTransaction({ to: ctx.falloutContract.address, value })
      ).to.not.be.reverted;

      await expect(ctx.falloutContract.connect(ctx.user1).withdrawEth()).to.not
        .be.reverted;
    });

    it("should withdraw contract balance to the correct withdrawal addresses in correct division.", async () => {
      ...
      await expect(
        ctx.falloutContract.setBeneficiaries(
          [
            ctx.user1.address,
            ctx.user2.address,
            ctx.user3.address,
      ...
      await expect(ctx.falloutContract.withdrawEth()).to.be.not.be.reverted;
  ...
  describe("When withdrawing ERC20 Funds", async () => {
    it("should fail to withdraw when balance is zero", async () => {
      await expect(
        ctx.falloutContract.withdrawErc20(ctx.standardERC20.address)
      ).to.be.revertedWith("ZeroBalance");
    });

    it("should allow anyone to initiate withdrawErc20(standardERC20.address) to default withdrawal addresses", async () => {
      const value = ethers.utils.parseEther("3.14159265358979323");

      await expect(
        ctx.standardERC20
          .connect(ctx.approved)
          .transfer(ctx.falloutContract.address, value)
      ).to.not.be.reverted;
      ...
      await expect(ctx.falloutContract.withdrawErc20(ctx.standardERC20.address))
        .to.not.be.reverted;
    });
